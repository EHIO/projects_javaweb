package org.wg.servlet.download;
...
public class DownloadServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest req, HttpServletResponse resp)
		 ...
		 * 3. 流：下载文件的数据
		 */
        String filename = "F:/temp/友谊天长地久.mp3";

        // 为了使下载框中显示中文文件名称不出乱码！
//		String framename = new String("友谊天长地久.mp3".getBytes("GBK"), "ISO-8859-1");
        String framename = filenameEncoding("yytcdj.mp3", req);

        String contentType = this.getServletContext()
                .getMimeType(filename);//通过文件名称获取MIME类型
        String contentDisposition = "attachment;filename=" + framename;//给出下载框中的文件名称
        // 一个流
